...
import com.example.demo3.dto.ApiResponse2;
...
public class ApiService {
    ...
    private String serviceKey2; // application.properties에 설정된 서비스 키 값을 주입받음

    public ApiService(ApiRepository apiRepository, ApiRepository2 apiRepository2) {
        ...
        this.apiRepository2 = apiRepository2;
    }
    ...
    public Mono<Void> getDrugInfoAndSave2(String pageNo) {
        // API 요청을 보낼 때 사용할 URI를 만드는 부분
        URI uri = UriComponentsBuilder
                .fromUriString("http://apis.data.go.kr/1471000/DrugPrdtPrmsnInfoService06/getDrugPrdtPrmsnDtlInq05")
        ...
        return webClient.get()
                ...
                .flatMap(apiResponse2 -> {
                    // 응답 본문에서 실제 데이터(items)를 꺼내고 각각을 ApiEntity로 변환
                    List<ApiEntity2> entities = apiResponse2.getBody().getItems().stream()
                            .map(item -> ApiEntity2.builder()
                                    .ITEM_NAME(item.getITEM_NAME())
                                    .ITEM_ENG_NAME(item.getITEM_ENG_NAME())
                                    .build())
                            .toList();
                    ...
                    return apiRepository2.saveAll(entities).then();
                });
    }
